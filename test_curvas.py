...
from utils.utils import TEMPLATE, ORGAN_NAME, NUM_CLASS,ORGAN_NAME_LOW_CURVAS
from utils.utils import organ_post_process_curvas, threshold_organ,create_entropy_map,save_soft_pred,invert_transform
...
def validation(model, ValLoader, val_transforms, args):
    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        if args.original_label:
            image, label, name_lbl,name_img = batch["image"], batch["label"], batch["name_lbl"],batch["name_img"]
            ...
            image,name_img = batch["image"],batch["name_img"]
            image_file_path = os.path.join(args.data_root_path,name_img[0] +'.nii.gz')
            case_save_path = os.path.join(save_dir,name_img[0].split('/')[0],name_img[0].split('/')[-1])
            pseudo_label_save_path = os.path.join(case_save_path,'backbones',args.backbone)
            if not os.path.isdir(pseudo_label_save_path):
        ...
        with torch.no_grad():
            ...
            pred = sliding_window_inference(inputs=image, 
        ...
        B = pred_hard.shape[0]
        for b in range(B):
            organ_list_all = TEMPLATE['curvas'] # post processing all organ
            pred_hard_post = organ_post_process_curvas(pred_hard.numpy(), organ_list_all,case_save_path,args)
        ...
        if args.store_entropy:
            organ_index_target = TEMPLATE['curvas']
            ...
            for organ_idx in organ_index_target:
                ...
                organ_name_target = ORGAN_NAME_LOW_CURVAS[organ_idx-1]
